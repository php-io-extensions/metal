...
#include "src/mtl-commandencoder.h"
...
{
	ZEPHIR_REGISTER_CLASS(Metal\\MTL\\MTLCommandEncoder, MTLCommandEncoder, metal, mtl_mtlcommandencoder_mtlcommandencoder, metal_mtl_mtlcommandencoder_mtlcommandencoder_method_entry, 0);
...
PHP_METHOD(Metal_MTL_MTLCommandEncoder_MTLCommandEncoder, endEncoding)
{
	...
	ZVAL_LONG(&_0, handle);
	mtl_mtlcommandencoder_end_encoding(&_0);
}
...
PHP_METHOD(Metal_MTL_MTLCommandEncoder_MTLCommandEncoder, label)
{
	...
	ZVAL_LONG(&_0, handle);
	mtl_mtlcommandencoder_label(&result, &_0);
	RETURN_CCTOR(&result);
}
...
PHP_METHOD(Metal_MTL_MTLCommandEncoder_MTLCommandEncoder, setLabel)
{
	...
	ZVAL_LONG(&_0, handle);
	mtl_mtlcommandencoder_set_label(&_0, label);
}
