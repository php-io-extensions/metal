...
3. This package is **Darwin-only** and has **zero coupling** to SDL3/GLFW — do not add those dependencies.
4. Public PHP API: `Metal\MTL\{App,Window,Menu,Device}` static methods wrapping `src/metal-*.h`.
5. `Window::clear($w, $r, $g, $b, $a = 255)` uses **integer RGBA 0..255** (C ABI floats 0..1).
