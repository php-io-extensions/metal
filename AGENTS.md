...
4. Public PHP API: `Metal\MTL\{App,Window,Menu,Device,Texture,Input}` static methods wrapping `src/metal-*.h`.
5. `Window::clear` / `Texture::clear` use **integer RGBA 0..255** (C ABI floats 0..1). Offscreen textures are for headless engines/UIs (`Texture`, since **0.7.1**). Window blit is `getDevice` / `presentTexture` (since **0.7.2**). `Input` (since **0.7.2**) is AppKit/GameController — never SDL/GLFW.
6. Build gotchas: `Makefile.frag` for `.m`, `METAL_SHARED_LIBADD -framework …` (includes GameController + CoreGraphics), explicit `CAMetalLayer` assignment, prefer `make -j1`.
