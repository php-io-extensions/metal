#!/usr/bin/env python3
"""Rewrite ext/config.m4 for Darwin-only Metal/AppKit Objective-C build."""
...
CONFIG = r"""PHP_ARG_ENABLE(metal, whether to enable metal, [ --enable-metal   Enable Metal])
...
if test "$PHP_METAL" = "yes"; then
	...
	METAL_SHARED_LIBADD="$METAL_SHARED_LIBADD -framework Foundation -framework AppKit -framework Metal -framework QuartzCore -framework GameController -framework CoreGraphics"
